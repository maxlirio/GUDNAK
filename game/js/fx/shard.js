// SHARDSWORN — the Dragon's fire: the square splits, crystal is blasted out of
// it, and the crystal burns.
//
...
//     two lights here are short and low, and their reach is smaller than the
//     gap to the next square, so a chain lights six squares and not the board.
...
// disposes its map, so these survive.
const TEXES = new Map();
function tex(key, paint) {
  let t = TEXES.get(key);
  if (!t) {
